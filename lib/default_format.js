...
var counters = function (key, value, ts, bucket) {
    ...
    var act = listKeys.slice(8, listKeys.length).join('.');
    bucket.push({
		"ns": listKeys[0] || '',
		"grp":listKeys[1] || '',
		"tgt":listKeys[2] || '',
...
var timer_data = function (key, value, ts, bucket) {
    ...
    value["tgt2"] = listKeys[3] || '';
    value["tgt3"] = listKeys[4] || '';
    value["tgt4"] = listKeys[5] || '';
    value["tgt5"] = listKeys[6] || '';
    value["tgt6"] = listKeys[7] || '';
    value["act"] = act || '';
    if (value['histogram']) {
